...
console.log(props.item,'prop');
  ...
  return (
   ...
   <Col className="mt-5 "  sm={6} xs={12} md={6} lg={3}>
   <div className="card">
   ...
   <p style={{color:'black',fontSize:'18px'}}> { props.item.fullName}: الإسم</p>

   <p style={{color:'black',fontSize:'15px'}}> { props.item.email}:بريد إلكتروني</p>

    
<AppPlaylist test={props.item} />
 
   <button className="btnCard" style={{marginTop:'5px'}}
   onClick={
    () => {tog_delete(true);
    set_id(props.item._id)
    }}
   >إحذف</button> 
 </div>
   ...
   {modal_delete && (
                    <DeleteModal
                      ...
                      _id={_id}
                      onDeleteClick={() => setmodal_delete(false)}
                    />
                  )}
